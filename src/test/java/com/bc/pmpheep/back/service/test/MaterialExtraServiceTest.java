...
public class MaterialExtraServiceTest extends BaseTest {
    Logger                       logger        =
                                               LoggerFactory.getLogger(MaterialExtraServiceTest.class);

    @Resource
    private MaterialExtraService materialExtraService;

    Random                       r             = new Random();
    MaterialExtra                materialExtra = new MaterialExtra(new Long(r.nextInt(200)),
                                                                   "notice", "note");
    ...
    public void testDeleteMaterialExtraById() {
        ...
        Assert.assertTrue("删除失败",
                          materialExtraService.deleteMaterialExtraById(materialExtra.getId()) > 0);
        // 删除
    ...
    public void testGetMaterialExtraById() {
        ...
        Assert.assertNotNull("获取数据失败",
                             materialExtraService.getMaterialExtraById(materialExtra.getId()));
    ...
    public void testGetMaterialId() {
        ...
        // 查询
        Assert.assertNotNull("获取数据失败",
                             materialExtraService.getMaterialExtraByMaterialId(materialExtra.getMaterialId()));
        materialExtraService.getMaterialExtraByMaterialId(r.nextLong());
    }
