...
public class SurveyServiceTest extends BaseTest {
    ...
    Survey        survey = new Survey("测试标题", "测试副标题", "测试简介", 1L, 2L, 1L, null, null, 999,
                                      (short) 0, false, null, null);

    private Survey addSurveys() {
        Survey survey =
        surveyService.addSurvey(new Survey("测试标题1", "测试副标题1", "测试简介1", 3L, 5L, 7L, null, null, 977,
                                           (short) 0, false, null, null));
        return survey;
    }
    ...
    public void updateSurvey() {
        ...
        Assert.assertTrue("更新失败", surveyService.updateSurvey(survey) > 0);
    }
    ...
    public void getSurveyById() {
        surveyService.addSurvey(survey);
        Assert.assertNotNull("获取数据失败", surveyService.getSurveyById(survey.getId()));
    }
