...
public class PmphGroupMessageServiceImpl extends BaseService implements PmphGroupMessageService {
     ...
     */
    @Override
    public PmphGroupMessage addPmphGroupMessage(PmphGroupMessage pmphGroupMessage)
    throws CheckedServiceException {
        if (null == pmphGroupMessage.getMsgContent()) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "消息内容为空");
        }
     ...
     */
    @Override
    public String deletePmphGroupMessageById(Long id, String sessionId)
    throws CheckedServiceException, IOException {
        ...
        if (num > 0) {// 删除成功 撤销消息
            ...
            List<PmphGroupMemberVO> list =
            pmphGroupMemberService.listPmphGroupMember(groupId, sessionId);
            List<String> ids = new ArrayList<String>();
            for (PmphGroupMemberVO groupMemberVO : list) {
                String tempId =
                (groupMemberVO.getIsWriter() ? "2" : "1") + "_" + groupMemberVO.getUserId();
                ids.add(tempId);
            }
     ...
     */
    @Override
    public Integer updatePmphGroupMessage(PmphGroupMessage pmphGroupMessage)
    throws CheckedServiceException {
    ...
    @Override
    public String addGroupMessage(String msgConrent, Long groupId, String sessionId,
    Short senderType) throws CheckedServiceException, IOException {
        ...
        pmphGroupMemberService.getPmphGroupMemberByMemberId(groupId, userId, false);// 获取后台用户
        ...
        new WebScocketMessage(String.valueOf(pmphGroupMessage.getId()), Const.MSG_TYPE_3, userId,
                              ...
                              Const.SEND_MSG_TYPE_0, null, null, msgConrent,
    ...
    @Override
    public PageResult<PmphGroupMessageVO> listPmphGroupMessage(
    PageParameter<PmphGroupMessageVO> pageParameter) throws CheckedServiceException {
        if (null == pageParameter.getParameter().getGroupId()) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "小组id为空");
        }
        if (null == pageParameter.getParameter().getGmtCreate()) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "进入小组时间为空");
        }
