...
public final class SystemMessageService {
    ...
    boolean isOnlyManager) throws CheckedServiceException, IOException {
        ...
        if (null == ids || ids.size() == 0) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MESSAGE,
                                              CheckedExceptionResult.NULL_PARAM, "消息接收对象为空");
        }
        // 向教师发送消息
        if (!isOnlyManager) {
            ...
            if (null != writerUserList && writerUserList.size() > 0) {
                ...
                new ArrayList<UserMessage>(writerUserList.size());
    ...
    public void sendWhenConfirmFirstEditor(Long bookId) throws CheckedServiceException, IOException {
        ...
        if (null == material) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MATERIAL,
                                              CheckedExceptionResult.NULL_PARAM, "该书籍没有找到对应的教材");
        }
        // 获取这本书的申报遴选列表
        ...
        for (DecPosition decPosition : decPositionLst) {
            if (null != decPosition && null != decPosition.getChosenPosition()
                && null != decPosition.getRank() && decPosition.getChosenPosition() == 1
                && decPosition.getRank() == 1) {// 筛选出主编
                Declaration declaration =
                declarationService.getDeclarationById(decPosition.getDeclarationId());
                // 消息内容
                String msgContent =
                "恭喜您被选为《<font color='red'>" + material.getMaterialName()
                + "</font>》<font color='red'>[" + textbook.getTextbookName()
                + "</font>]的第一主编，您现在可以开始遴选编委了";
     ...
     * @throws IOException
     */
    public void sendWhenInviteJoinGroup(String inviterName, Long groupId,
    List<Long> invitedPersonIds, short invitedPersonType) throws CheckedServiceException,
    IOException {
        if (StringUtils.isEmpty(inviterName)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "邀请人为空");
        }
        if (null == invitedPersonIds || invitedPersonIds.size() == 0) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "被邀请人为空");
        }
        // 获取小组
        PmphGroup pmphGroup = pmphGroupService.getPmphGroupById(groupId);
        if (null == pmphGroup) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "小组不存在");
        }
        String msgContent =
        "您被[<font color='red'>" + inviterName + "</font>]邀请加入[<font color='red'>"
     ...
     * @throws IOException
     */
    public void sendWhenQuitGroup(String exitPersonName, Long groupId)
    throws CheckedServiceException, IOException {
        if (StringUtils.isEmpty(exitPersonName)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "退出人为空");
        }
        // 获取小组
        PmphGroup pmphGroup = pmphGroupService.getPmphGroupById(groupId);
        if (null == pmphGroup) {
        ...
        List<PmphGroupMemberVO> PmphGroupMemberVOList =
        pmphGroupMemberService.listPmphGroupMember(groupId, null);
        if (null == PmphGroupMemberVOList || PmphGroupMemberVOList.size() == 0) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "小组没有成员");
        }
        // 存入消息主体
        String msgContent =
        "[<font color='red'>" + exitPersonName + "</font>]退出了[" + pmphGroup.getGroupName() + "]小组";
        Message message = new Message(msgContent);
        message = messageService.add(message);
        String msg_id = message.getId();
        // 组装消息和消息对象
        List<UserMessage> userMessageList =
        new ArrayList<UserMessage>(PmphGroupMemberVOList.size());
     ...
     * @throws IOException
     */
    public void sendWhenTeacherCertificationAudit(String auditorOrgName, List<Long> teacherIds,
    boolean isPass) throws CheckedServiceException, IOException {
        if (StringUtils.isEmpty(auditorOrgName)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.TEACHER_CHECK,
                                              CheckedExceptionResult.NULL_PARAM, "审核机构为空");
        }
        if (null == teacherIds || teacherIds.size() == 0) {
            throw new CheckedServiceException(CheckedExceptionBusiness.TEACHER_CHECK,
                                              CheckedExceptionResult.NULL_PARAM, "认证的教师为空");
        }
        // 存入消息主体
        String msgContent =
        "抱歉，您提交的教师认证资料已被[<font color='red'>" + auditorOrgName + "</font>]管理员退回，请您核对后重试";// 退回
        if (isPass) {// 通过
     ...
     * @throws IOException
     */
    public void sendWhenSubmitDeclarationForm(Long declarationId) throws CheckedServiceException,
    IOException {
        // 获取申报表
        ...
        if (declaration.getOrgId() == 0) {// 提交的人卫社
            // 项目编辑 策划编辑 主任 id
            String msgContent =
            "[<font color='red'>" + declaration.getRealname() + "</font>]提交了《<font color='red'>"
            ...
            String msgContent =
            "贵校老师[<font color='red'>" + declaration.getRealname()
            + "</font>]提交了《<font color='red'>" + material.getMaterialName()
            + "</font>》申报表，请及时进行资料审核、打印并快递申报纸质表";
            // 存入消息主体
            Message message = new Message(msgContent);
            message = messageService.add(message);
            String msg_id = message.getId();
    ...
    throws CheckedServiceException, IOException {
        ...
        if (declaration.getOrgId() == 0) {// 提交的人卫社
            msgContent =
            "抱歉，您提交的《<font color='red'>" + material.getMaterialName()
            + "</font>》申报表被[<font color='red'>出版社</font>]退回，请您核对后重试";
            if (isPass) {// 通过
                msgContent =
                "恭喜！您提交的《<font color='red'>" + material.getMaterialName()
    ...
    */
     ...
     * @throws IOException
     */
    public void sendWhenReceiptAudit(Long declarationId, boolean isPass)
    throws CheckedServiceException, IOException {
        Declaration declaration = declarationService.getDeclarationById(declarationId);
        if (null == declaration) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MATERIAL,
        ...
        if (null == declaration.getOrgId()) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MATERIAL,
                                              CheckedExceptionResult.NULL_PARAM, "管理员为空");
        }
        Material material = materialService.getMaterialById(declaration.getMaterialId());
        String orgMsgContent =
        "抱歉，贵校老师[<font color='red'>" + declaration.getRealname() + "</font>]在《<font color='red'>"
        + material.getMaterialName() + "</font>》提交的申报纸质表被退回";
        String writerMsgContent =
        "抱歉，您在《<font color='red'>" + material.getMaterialName() + "</font>》提交的申报纸质表被退回，请您核对后重试";
        if (isPass) {// 收到
            orgMsgContent =
            "您好，人民卫生出版社已收到贵校老师[<font color='red'>" + declaration.getRealname()
            + "</font>]在《<font color='red'>" + material.getMaterialName() + "</font>》提交的申报纸质表";
            writerMsgContent =
            "您好，人民卫生出版社已收到您在《<font color='red'>" + material.getMaterialName()
    ...
    throws CheckedServiceException, IOException {
        ...
        if (null == cmsContent) {
            throw new CheckedServiceException(CheckedExceptionBusiness.CMS,
                                              CheckedExceptionResult.NULL_PARAM, "文章不存在");
        }
        String msgContent =
        "抱歉，您的文章《<font color='red'>" + cmsContent.getTitle() + "</font>》没有通过审核，请您修改后重试 ";
        if (isPass) {
    ...
    public void sendWhenPubfinalResult(Long textBookId) throws CheckedServiceException, IOException {
        // 获取这书根据书籍id获取申报职位
        List<DecPosition> decPositionList =
        decPositionService.listDecPositionsByTextbookId(textBookId);
        ...
        if (material.getIsAllTextbookPublished()) {// 所有都发布了
            String orgMsg =
            "《<font color='red'>" + material.getMaterialName()
            ...
            for (Org org : orgs) {
                ...
                decPositionService.listDecPositionsByTextbookIdAndOrgid(bookIds, org.getId());
                if (null != decPositions && decPositions.size() > 0) {
                    ...
                    for (int i = 0; i < decPositions.size(); i++) {
                        ...
                        declarationService.getDeclarationById(decPosition.getDeclarationId());
                        msgContent +=
                        "</br>" + "[<font color='red'>" + declaration.getRealname() + "</font>]";
                        msgContent +=
                        " - "
                        + textbookService.getTextbookById(decPosition.getTextbookId())
                                         .getTextbookName() + " - ";
