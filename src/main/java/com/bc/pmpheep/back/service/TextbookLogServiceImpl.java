	...
	public  void  addTextbookLog(List<DecPosition> oldlist ,Long textbookId,Long updaterId,int userType) throws CheckedServiceException{
		...
		for(DecPosition newDecPosition :newlist){
			...
			if(null != newChosenPosition && newChosenPosition == 1 ){
				...
				for(DecPosition oldDecPosition :oldlist){
					temp.append(oldDecPosition.getDeclarationId()+"_"+oldDecPosition.getChosenPosition()+",");
				}
		...
		for(DecPosition oldDecPosition :oldlist){
			...
			if(null != oldChosenPosition && oldChosenPosition == 1 ){
				StringBuilder temp = new StringBuilder(",");
				for(DecPosition newDecPosition :newlist){
					temp.append(newDecPosition.getDeclarationId()+"_"+newDecPosition.getChosenPosition()+",");
				}
		...
		if( addSumZhuBian >0 || redSumZhuBian >0 || addSumFuZhuBian    >0 || redSumFuZhuBian >0 ||
			addSumBianWei >0 || redSumBianWei >0 || addSumShuZiBianWei >0 || redSumShuZiBianWei >0 ){
