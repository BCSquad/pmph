...
public class PmphGroupServiceImpl extends BaseService implements PmphGroupService {
    ...
    public PmphGroup addPmphGroup(PmphGroup pmphGroup) throws CheckedServiceException {
        ...
        if (StringUtil.isEmpty(pmphGroup.getGroupName())) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                    CheckedExceptionResult.NULL_PARAM, "小组名称为空不允许新增");
        }
     ...
     */
    @Override
    public String deletePmphGroupById(PmphGroup pmphGroup, String sessionId)
            throws CheckedServiceException {
        String result = "FAIL";
        if (pmphGroupMemberService.isFounder(pmphGroup.getId(), sessionId)) {
            ...
            List<PmphGroupFile> pmphGroupFiles
                    = pmphGroupFileService.listPmphGroupFileByGroupId(pmphGroup.getId());// 获取该小组的文件
    ...
    public PmphGroup addPmphGroupOnGroup(MultipartFile file, PmphGroup pmphGroup, String sessionId)
        ...
        if (null != pmphGroup.getId()) {// 判断是否新增小组成功，如果成功则调用PmphGroupMemberService添加小组成员的方法将创建者添加到小组中
            ...
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                    CheckedExceptionResult.OBJECT_NOT_FOUND, "添加小组失败");
        }
        return pmphGroup;
    }
    ...
    public PmphGroup updatePmphGroupOnGroup(MultipartFile file, PmphGroup pmphGroup,
        ...
        if (null == pmphGroup) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                    CheckedExceptionResult.NULL_PARAM, "参数对象不能为空");
        }
        if (null == pmphGroup.getId()) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                    CheckedExceptionResult.ILLEGAL_PARAM, "小组id不能为空");
        }
