...
public class PmphGroupMemberServiceImpl extends BaseService implements PmphGroupMemberService {
    ...
    throws CheckedServiceException {
        ...
        for (PmphGroupMemberVO pmphGroupMemberVO : list) {
            if (pmphGroupMemberVO.getIsWriter()) {
                pmphGroupMemberVO.setAvatar(RouteUtil.userAvatar(writerUserService.get(pmphGroupMemberVO.getUserId())
                                                                                  .getAvatar()));
                pmphGroupMemberVO.setUserType(Const.SENDER_TYPE_2);
            } else {
                pmphGroupMemberVO.setAvatar(RouteUtil.userAvatar(pmphUserService.get(pmphGroupMemberVO.getUserId())
                                                                                .getAvatar()));
        ...
        if (pmphUser.getIsAdmin() || isFounderOrisAdmin(groupId, sessionId)) {// 是超级管理员或者该小组的创建人和管理员才可以添加成员
            if (pmphGroupMembers.size() > 0) {
                ...
                try {
                    if (!writers.isEmpty()) {
                        // 向作家用户发送消息
                        systemMessageService.sendWhenInviteJoinGroup(pmphUser.getRealname(),
                                                                     groupId,
                                                                     writers,
                                                                     (short) 2);
                    }
    ...
    throws CheckedServiceException {
        ...
        PmphGroupMemberVO currentUser =
        pmphGroupMemberDao.getPmphGroupMemberByMemberId(groupId, userid, false);
            ...
            for (Long id : ids) {
                ...
                if (pmphUser.getIsAdmin() || currentUser.getIsFounder()) {// 只有小组创建者和超级管理员可以删除小组成员
                    ...
                    if (currentUser.getIsAdmin()
                        && (pmphGroupMember.getIsFounder() || pmphGroupMember.getIsAdmin())) {
                        throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                                          CheckedExceptionResult.ILLEGAL_PARAM,
                                                          "您无权限删除管理员，请重新选择");
                    } else {
    ...
    @Override
    public String updateGroupMemberByIds(Long groupId, Long[] ids, String sessionId)
    throws CheckedServiceException {
        String result = "FAIL";
        PmphUser pmphUser = SessionUtil.getPmphUserBySessionId(sessionId);
        if (null == pmphUser || null == pmphUser.getId()) {
    ...
    @Override
    public PmphGroupMemberVO getPmphGroupMemberByMemberId(Long groupId, Long userId,
    Boolean isWriter) throws CheckedServiceException {
        if (null == userId) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "组员id不能为空");
        }
        PmphGroupMemberVO pmphGroupMemberVO =
        pmphGroupMemberDao.getPmphGroupMemberByMemberId(groupId, userId, isWriter);
        if (ObjectUtil.isNull(pmphGroupMemberVO)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                              CheckedExceptionResult.NULL_PARAM, "你不是该小组的组员");
        }
        pmphGroupMemberVO.setAvatar(isWriter ? writerUserService.get(userId).getAvatar() : pmphUserService.get(userId)
                                                                                                          .getAvatar());
        return pmphGroupMemberVO;
    }
        ...
        if (pmphUser.getIsAdmin() || isFounder(groupId, sessionId)) {// 只有小组创建者与超级管理员可以修改小组人员权限
            if (null == members || members.size() == 0) {
                ...
                for (PmphGroupMember pmphGroupMember : members) {
                    PmphGroupMember member =
                    pmphGroupMemberDao.getPmphGroupMemberById(pmphGroupMember.getId());
    ...
    throws CheckedServiceException {
        ...
        // }
        // // 重新创建小组
        // //PmphGroup group = pmphGroupService.addEditorSelcetionGroup(sessionId, list,
        // textbookId);
        // throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
        // CheckedExceptionResult.NULL_PARAM, "小组已被删除，现在重新创建小组");
        // }
        // 通过小组id查询小组现有成员
