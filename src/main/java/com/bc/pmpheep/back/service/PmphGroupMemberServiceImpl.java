...
public class PmphGroupMemberServiceImpl extends BaseService implements PmphGroupMemberService {
    ...
    @Override
    public String addPmphGroupMemberOnGroup(List<PmphGroupMember> pmphGroupMembers)
    throws CheckedServiceException {
        String result = "FAIL";
    ...
    public Boolean isFounderOrisAdmin() throws CheckedServiceException {
        boolean flag = false;
        PmphUser pmphUser =
        (PmphUser) (ShiroSession.getShiroSessionUser().getAttribute(Const.SESSION_PMPH_USER));
        ...
        if (null == ids || ids.size() == 0) {
            ...
            for (Long id : ids) {
                ...
                if (currentUser.isIsAdmin()
                    && (pmphGroupMemberDao.getPmphGroupMemberById(id).isIsFounder() || pmphGroupMemberDao.getPmphGroupMemberById(id)
                                                                                                         .isIsAdmin())) {
                    throw new CheckedServiceException(CheckedExceptionBusiness.GROUP,
                                                      CheckedExceptionResult.ILLEGAL_PARAM,
                                                      "管理员不能删除创建者或其他管理员");
