...
public class PmphUserServiceImpl implements PmphUserService {
	...
	@Override
	public Page<PmphUserManagerVO, Map<String, String>> getListPmphUser(
			Page<PmphUserManagerVO, Map<String, String>> page) throws CheckedServiceException {
		if (null != page.getParameter().get("name")) {
			String name = page.getParameter().get("name").trim();
			if (!name.equals("")) {
				page.getParameter().put("name", "%" + name + "%");
			} else {
				page.getParameter().put("name", name);
			}
		}
		if (null != page.getParameter().get("path")) {
			String path = page.getParameter().get("path").trim();
			if (!path.endsWith("")) {
				page.getParameter().put("path", path + "%");
			} else {
				page.getParameter().put("path", path);
			}
		}
	...
	@Override
	public String updatePmphUserOfBack(PmphUser pmphUser) throws CheckedServiceException {
		if (null == pmphUser.getId()) {
			throw new CheckedServiceException(CheckedExceptionBusiness.USER_MANAGEMENT,
					CheckedExceptionResult.NULL_PARAM, "用户ID为空时禁止更新用户");
		}
		int num = userDao.update(pmphUser);
		String result = "FAIL";
		if (num > 0) {
