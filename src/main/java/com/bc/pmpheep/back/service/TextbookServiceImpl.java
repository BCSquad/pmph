...
public class TextbookServiceImpl implements TextbookService {
    ...
    @Override
    public PageResult<BookPositionVO> listBookPosition(Integer pageNumber, Integer pageSize,
    Integer state, String textBookIds, String bookName, Long materialId, String sessionId) {
        ...
        if (null == power) {
            ...
            if (null != materialProjectEditors && materialProjectEditors.size() > 0) {
                for (MaterialProjectEditorVO materialProjectEditor : materialProjectEditors) {
                    if (null != materialProjectEditor
                        && null != materialProjectEditor.getEditorId()
        ...
        if (null == power) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MATERIAL,
                                              CheckedExceptionResult.NULL_PARAM, "该教材您没操作权限");
        }
            ...
            for (BookPositionVO row : rows) {
                ...
                if (power == 1 || power == 2) { // 管理员或者主任
                    ...
                    if (null != row && null != row.getPlanningEditor() && null != pmphUser.getId()
                        ...
                        tempProjectPermission =
                        (tempProjectPermission | material.getPlanPermission());
                    }
    ...
    public Integer updateTextbooks(Long[] ids, String sessionId) {
        ...
        if (textbooks.size() > 0) {
            for (Textbook textbook : textbooks) {
                ...
                List<DecPosition> decPosition =
                decPositionService.getDecPositionByTextbookId(textbook.getId());
                // 是否确认编委
    ...
    public List<BookListVO> getBookListVOs(Long materialId) throws CheckedServiceException {
        ...
        for (int i = 0; i < pathType.length; i++) {
            String type =
            materialTypeService.getMaterialTypeById(Long.valueOf(pathType[i])).getTypeName();
    ...
    @Override
    public List<Textbook> addOrUpdateTextBookList(Long materialId, Boolean isPublic,
    String textbooks, String sessionId) throws CheckedServiceException {
        if (ObjectUtil.isNull(materialId)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.MATERIAL,
                                              CheckedExceptionResult.NULL_PARAM, "教材id不能为空");
        }
    ...
    @Override
    public List<Textbook> importExcel(MultipartFile file, Long materialId)
    throws CheckedServiceException {
        if (ObjectUtil.isNull(materialId)) {
            throw new CheckedServiceException(CheckedExceptionBusiness.TEXTBOOK,
                                              CheckedExceptionResult.NULL_PARAM, "教材id不能为空");
            ...
            for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
                ...
                if (null == noPeople || noPeople.isEmpty()) {
                    ...
                    for (Textbook book : noPeople) {
                        if (sort.intValue() == book.getSort()
                            && round.intValue() == book.getTextbookRound()
    ...
    @Override
    public Integer updatRevisionTimesByTextBookId(Integer number, Long textBookId)
    throws CheckedServiceException {
