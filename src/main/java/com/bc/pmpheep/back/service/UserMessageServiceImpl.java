...
public class UserMessageServiceImpl extends BaseService implements UserMessageService {
    ...
    String userIds, String bookIds, boolean isSave) throws CheckedServiceException, IOException {
        ...
        if (sendType == 1 || sendType == 2) {
            ...
            for (OrgUser orgUser : orgUserList) {
                userMessageList.add(new UserMessage(message.getId(), (short) 1, senderId,
                                                    (short) 1, orgUser.getId(), (short) 3));
            }
            // 作家用户
            if (sendType == 2) {
                ...
                for (WriterUser writerUser : writerUserList) {
                    userMessageList.add(new UserMessage(message.getId(), (short) 1, senderId,
                                                        (short) 1, writerUser.getId(), (short) 2));
                }
            }
        }
        // 3 指定用户
        ...
        if (null != websocketUserIds && websocketUserIds.size() > 0) {
            ...
            new WebScocketMessage(message.getId(), (short) 1, senderId, pmphUser.getRealname(),
                                  (short) 1, (short) 0, message.getContent(), message.getContent(),
