...
public class UserMessageServiceImpl extends BaseService implements UserMessageService {
	...
	public Map<String, Object> listSendOject(Integer sendType, Integer pageNumber, Integer pageSize, String orgName,
			Long materialId, String userNameOrUserCode, String materialName) throws CheckedServiceException {
		...
		if (Const.SEND_OBJECT_1.intValue() == sendType.intValue()
			...
			for (OrgUser orgUser : orgUserList) {
				userMessageList.add(new UserMessage(message.getId(), title, Const.MSG_TYPE_1, senderUserId,
						Const.SENDER_TYPE_1, orgUser.getId(), Const.RECEIVER_TYPE_3, 0L));
			}
			// 作家用户
			if (Const.SEND_OBJECT_2.intValue() == sendType.intValue()) {
				...
				for (WriterUser writerUser : writerUserList) {
					userMessageList.add(new UserMessage(message.getId(), title, Const.MSG_TYPE_1, senderUserId,
							Const.SENDER_TYPE_1, writerUser.getId(), Const.RECEIVER_TYPE_2, 0L));
				}
			}
		}
		// 3 指定用户
			...
			for (String id : ids) {
				if (StringUtil.notEmpty(id)) {
					...
					if (StringUtil.notEmpty(userId) && StringUtil.isNumeric(userId)) {
						userMessageList.add(new UserMessage(message.getId(), title, Const.MSG_TYPE_1, senderUserId,
								Const.SENDER_TYPE_1, Long.parseLong(userId), new Short(userType), 0L));
					}
				}
			}
		}
		...
		if (Const.SEND_OBJECT_4.intValue() == sendType.intValue()) {
			...
			for (String id : ids) {
				...
				for (Long userId : userIdList) {
					userMessageList.add(new UserMessage(message.getId(), title, Const.MSG_TYPE_1, senderUserId,
							Const.SENDER_TYPE_1, userId, Const.RECEIVER_TYPE_2, 0L));
				}
				...
				if (file.isFile()) {
					...
					if (StringUtil.notEmpty(file.getName())) {
						String gridFSFileId = fileService.saveLocalFile(file, FileType.MSG_FILE,
								CastUtil.castLong(msgId));// 上传文件到MongoDB
						if (StringUtil.isEmpty(gridFSFileId)) {
							throw new CheckedServiceException(CheckedExceptionBusiness.MESSAGE,
									CheckedExceptionResult.FILE_UPLOAD_FAILED, "文件上传失败!");
						}
						// 保存对应数据
						MessageAttachment mAttachment = messageAttachmentService
								.addMessageAttachment(new MessageAttachment(msgId, gridFSFileId, file.getName()));
	...
	public MyMessageVO updateMyMessageDetail(Long id) throws CheckedServiceException {
		...
		myMessageVO.setContent(message.getContent());
		List<MessageAttachment> messageAttachments = messageAttachmentService
				.getMessageAttachmentByMsgId(myMessageVO.getMsgId());
