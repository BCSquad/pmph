...
public class GroupController {
    ...
    public ResponseBean addPmphGroupMemberOnGroup(Long gruopId, ListPar listPar, String sessionId) {
        return new ResponseBean(
                                pmphGroupMemberService.addPmphGroupMemberOnGroup(gruopId,
                                                                                 listPar.getPmphGroupMembers(),
                                                                                 sessionId));
    }
    ...
    String sessionId) {
        try {
            return new ResponseBean(pmphGroupService.updatePmphGroupOnGroup(file,
                                                                            pmphGroup,
                                                                            sessionId));
        } catch (IOException e) {
            return new ResponseBean(e);
    ...
    public ResponseBean deletePmphGroupFileById(Long groupId, ListPar listPar, String sessionId) {
        return new ResponseBean(pmphGroupFileService.deletePmphGroupFileById(groupId,
                                                                             listPar.getIds(),
                                                                             sessionId));
    }
     ...
     * 功能描述：批量删除小组成员
     * 
     * @param listPar 小组成员id
     * @param sessionId 当前操作者
     * @return
     * 
     */
    @ResponseBody
    ...
    public ResponseBean deletePmphGroupMemberByIds(Long groupId, ListPar listPar, String sessionId) {
        return new ResponseBean(pmphGroupMemberService.deletePmphGroupMemberByIds(groupId,
                                                                                  listPar.getIds(),
                                                                                  sessionId));
    }
    ...
    public ResponseBean updateMemberIdentity(Long groupId, ListPar listPar, String sessionId) {
        ...
        return new ResponseBean(pmphGroupMemberService.updateMemberIdentity(groupId,
                                                                            list,
                                                                            sessionId));
    }
    ...
    public ResponseBean listGroupMemberManagerVOs(Integer pageSize, Integer pageNumber,
     ...
     * 功能描述：发送小组消息
     * 
     * @param msgConrent 消息内容
     * @param groupId 小组id
     * @param sessionId 当前用户session id
     * @return 是否成功
     * 
     */
    @ResponseBody
    ...
    public ResponseBean addGroupMessage(String msgConrent, Long groupId, String sessionId) {
        try {
            return new ResponseBean(pmphGroupMessageService.addGroupMessage(msgConrent,
                                                                            groupId,
                                                                            sessionId,
                                                                            Const.SENDER_TYPE_1));
        } catch (IOException e) {
    ...
    public ResponseBean deletePmphGroupMessageById(Long id, String sessionId) {
        try {
            return new ResponseBean(pmphGroupMessageService.deletePmphGroupMessageById(id,
                                                                                       sessionId));
        } catch (IOException e) {
            return new ResponseBean(e);
        }
     ...
     * @param groupId 小组id
     * @param nowTime 进入小组的时间节点
     * @return 消息结果集
     * 
     */
    @ResponseBody
    @RequestMapping(value = "/list/message", method = RequestMethod.GET)
    public ResponseBean listPmphGroupMessage(Integer pageSize, Integer pageNumber, Long groupId,
    Long baseTime) {
