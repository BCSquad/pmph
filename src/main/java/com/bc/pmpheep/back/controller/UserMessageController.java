...
public class UserMessageController {
	...
	public ResponseBean sendObject(@RequestParam("sendType") Integer sendType,
			...
			@RequestParam("materialId") Long materialId, @RequestParam("userNameOrUserCode") String userNameOrUserCode,
			@RequestParam("materialName") String materialName) {
		return new ResponseBean(userMessageService.listSendOject(sendType, pageNumber, pageSize, orgName, materialId,
				userNameOrUserCode, materialName));
	}
	...
	public ResponseBean newMessage(Message message, @RequestParam("title") String title,
			...
			@RequestParam("bookIds") String bookids, @RequestParam("file") String[] files, HttpServletRequest request) {
		try {
			String sessionId = CookiesUtil.getSessionId(request);
			return new ResponseBean(userMessageService.addOrUpdateUserMessage(message, title, sendType, orgIds,
					senderId, userIds, bookids, true, files, sessionId));
		} catch (IOException e) {
	...
	public ResponseBean again(Message message, @RequestParam("title") String title,
		...
		try {
			String sessionId = CookiesUtil.getSessionId(request);
			return new ResponseBean(userMessageService.addOrUpdateUserMessage(message, title, sendType, orgIds,
					senderId, userIds, bookIds, false, files, sessionId));
		} catch (IOException e) {
			return new ResponseBean(e);
		}
	}
	...
	public ResponseBean updateMessage(Message message, @RequestParam("msgId") String msgId,
		...
		try {
			return new ResponseBean(userMessageService.updateUserMessage(message, msgId, msgTitle, files, attachment));
		} catch (IOException e) {
			return new ResponseBean(e);
		}
	}
	...
	public ResponseBean myMessage(Integer pageSize, Integer pageNumber, String title, Boolean isRead, Long userId,
			Integer userType) {
	...
	public ResponseBean newOneUserMessage(Message message, @RequestParam("receiverId") Long receiverId,
			HttpServletRequest request) {
		try {
			String sessionId = CookiesUtil.getSessionId(request);
			return new ResponseBean(userMessageService.addOneUserMessage(message, receiverId, sessionId));
		} catch (IOException e) {
			return new ResponseBean(e);
		}
	}
