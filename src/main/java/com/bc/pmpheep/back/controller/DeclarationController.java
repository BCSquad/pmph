...
public class DeclarationController {
	...
	public ResponseBean declaration(@RequestParam(value = "pageNumber", required = true) Integer pageNumber,
		...
		return new ResponseBean(
				declarationService.pageDeclaration(pageNumber, pageSize, materialId, textBookids, realname, position,
						title, orgName, orgId, unitName, positionType, onlineProgress, offlineProgress, haveFile));
	}
	...
	public ResponseBean onlineProgress(@RequestParam("id") Long id,
			@RequestParam("onlineProgress") Integer onlineProgress, @RequestParam("returnCause") String returnCause)
			throws CheckedServiceException, IOException {
	...
	public ResponseBean saveBooks(DecPositionVO decPositionVO, HttpServletRequest request) throws IOException {
	...
	* &#64;return
	...
	public ResponseBean selection(@RequestParam("textbookId") Long textbookId,
		...
		return new ResponseBean(decPositionService.listEditorSelection(textbookId, materialId, realName, orgName));
	}
	...
	public ResponseBean update(@RequestParam("jsonDecPosition") String jsonDecPosition,
			...
			@RequestParam("editorOrEditorialPanel") Integer editorOrSubeditorType, HttpServletRequest request) {
		String sessionId = CookiesUtil.getSessionId(request);
		try {
			return new ResponseBean(decPositionService.updateDecPositionEditorSelection(jsonDecPosition, selectionType,
					editorOrSubeditorType, sessionId));
		} catch (IOException e) {
