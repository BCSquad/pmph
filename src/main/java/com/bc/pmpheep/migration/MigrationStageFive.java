...
public class MigrationStageFive {
	...
	private void textbook() {
		String sql="select a.*,b.new_pk newmaterid,ifnull(c.new_pk,d.new_pk) bookcreateuserid,b.createdate newcreatedate,e.new_pk editroid "
        ...
        for (Map<String, Object> map : maps) {
            ...
            if( null != revision){
            	...
            	map.put(SQLParameters.EXCEL_EX_HEADER, "书籍轮次为空");
            	excel.add(map);
            	logger.error("书籍轮次为空，此结果将被记录在Excel中");
            	continue;
            }
            ...
            if(null==createdate){
				...
				excel.add(map);
				logger.error("创建时间为空，此结果将将被记录在Excel中");
				continue;
            }
            ...
            textbook.setGmtCreate((Timestamp) (createdate));// 创建时间
