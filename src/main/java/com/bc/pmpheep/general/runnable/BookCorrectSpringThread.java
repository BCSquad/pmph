...
import com.bc.pmpheep.utils.BookCorrectWordHelper;
...
public class BookCorrectSpringThread implements Runnable {

	BookCorrectionService bookCorrectionService;
	BookCorrectionDao bookCorrectionDao;
	String bookname;
	Boolean isEditorReplied;
	Logger logger = LoggerFactory.getLogger(BookCorrectSpringThread.class);
	ZipHelper zipHelper;
...
//	ExpertationService expertationService;
	...
	public BookCorrectSpringThread(ZipHelper zipHelper,
								   BookCorrectWordHelper bookCorrectWordHelper,
                                   BookCorrectionService bookCorrectionService,
		...
		this.bookCorrectWordHelper = bookCorrectWordHelper;
...
//		System.out.println("线程开始");
		...
		String fname = FileUtil.replaceIllegalCharForFileName("图书纠错审核");
		...
		List<BookCorrectionAuditVO> list = new ArrayList<>();
		//业务逻辑获取数据
		try {
			PageResult<BookCorrectionAuditVO> n = bookCorrectionService.listBookCorrectionAudit(request, 1, -1, bookname, isEditorReplied, result);
			list = n.getRows();
		...
		try{
			for (int i = 0; i < list.size(); i++) {
				BookCorrectionAuditVO vo = list.get(i);
				if (!CollectionUtil.isEmpty(list)) {
					...
					this.bookCorrectWordHelper.export(vo.getBookname(),sb.toString(), vo,(i + 1));
				}
