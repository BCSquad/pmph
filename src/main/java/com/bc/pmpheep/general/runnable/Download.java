...
public class Download {
	 ...
	 *//*
		 ...
		 * InputStream is = null; OutputStream os = null; //存放路径 String savePath = null;
		 * //文件名称 String filename = null; List<String> listHtmlImgs = new
		 * ArrayList<String>(); for (String urlString : listImgSrc) { try { // 构造URL URL
		 * url = new URL(urlString); // 打开连接 URLConnection con = url.openConnection();
		 * // 设置请求超时为5s con.setConnectTimeout(5 * 1000); // 输入流 is =
		 * con.getInputStream(); // 1K的数据缓冲 byte[] bs = new byte[1024]; // 读取到的数据长度 int
		 * len; // 输出的文件流 File sf = new File(savePath); if (!sf.exists()) { sf.mkdirs();
		 * } os = new FileOutputStream(sf.getPath() + "\\" + filename); // 开始读取 while
		 * ((len = is.read(bs)) != -1) { os.write(bs, 0, len); } // 新建文件用于存放图片 File file
		 * = new File("D://2.png"); Random rand = new Random(); // 上传到芒果 返回芒果id String
		 * mongoId =fileService.saveLocalFile(file, FileType.CMS_IMG, (long)
		 * rand.nextInt(900) + 100); //把芒果id加到list集合中 listHtmlImgs.add(mongoId); //
		 * 删除文件夹以及图片 FileUtil.deleteDirectory("D:\\\\"); System.out.println(mongoId); }
		 * catch (Exception e) { e.printStackTrace(); } finally { // 完毕，关闭所有链接
		 * os.close(); is.close(); } } return listHtmlImgs; }
		 */
