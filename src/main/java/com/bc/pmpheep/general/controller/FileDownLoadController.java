...
public class FileDownLoadController {
    ...
    Integer onlineProgress, Integer offlineProgress) {
        ...
        taskExecutor.execute(new SpringThread(zipHelper, wordHelper, materialService,
                                              ...
                                              offlineProgress, id, materialExtensionService));
        return '"' + id + '"';
    }
    ...
    public String declarationWord(String selectedIds) {
        ...
        String.valueOf(System.currentTimeMillis())
              .concat(String.valueOf(RandomUtil.getRandomNum()));
        taskExecutor.execute(new Front(logger, zipHelper, wordHelper, materialService,
                                       textbookService, declarationService,
                                       materialExtensionService, decIds, id));
        return '"' + id + '"';
    }
    ...
    @ResponseBody
    public void orgUserExportEcel(HttpServletRequest request, HttpServletResponse response,
    String name, String orgName, String orgTypeName, Boolean isHospital) {
    ...
    @RequestParam("surveyId") Long surveyId, @RequestParam("questionId") Long questionId) {
        ...
        try {
            SurveyQuestionFillVO sqf = new SurveyQuestionFillVO();
            sqf.setQuestionId(questionId);
            sqf.setSurveyId(surveyId);
            PageParameter<SurveyQuestionFillVO> pageParameter = new PageParameter<>(1, 50000);
            pageParameter.setParameter(sqf);
